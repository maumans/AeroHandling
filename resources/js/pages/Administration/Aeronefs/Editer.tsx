...
interface Aeronef {
    ...
    code: string;
    modele: string;
    type_aeronef_id: number;
    categorie_aeronef_id: number;
...
export default function AdministrationAeronefsEditer({ aeronef, types, categories }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        code: aeronef.code,
        modele: aeronef.modele,
        type_aeronef_id: aeronef.type_aeronef_id?.toString() ?? '',
        categorie_aeronef_id: aeronef.categorie_aeronef_id?.toString() ?? '',
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            <Head title={`${t('Modifier')} — ${aeronef.code}`} />
            <div className="flex flex-col gap-6 p-4 md:p-6">
                <div className="flex items-center gap-3">
                    <Button variant="ghost" size="icon" asChild>
                        <Link href="/administration/aeronefs"><ArrowLeft className="size-4" /></Link>
                    </Button>
                    <h1 className="text-2xl font-bold">{t("Modifier l'aéronef")}</h1>
                </div>
                        ...
                        <form onSubmit={soumettre} className="flex flex-col gap-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <Select value={data.categorie_aeronef_id} onValueChange={(v) => setData('categorie_aeronef_id', v)}>
                                        <SelectTrigger>
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
